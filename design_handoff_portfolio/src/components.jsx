    ...
    <aside className="nav">
      ...
      <div className="nav-footer">
        ...
        <div className="theme-pill" role="group" aria-label="Theme mode">
          <button className={mode === 'light' ? 'active' : ''} onClick={() => setMode('light')} title="Light">
            <Icon.sun />
          </button>
          <button className={mode === 'dark' ? 'active' : ''} onClick={() => setMode('dark')} title="Dark">
            <Icon.moon />
          </button>
        </div>
...
function NowPlaying() {
  return (
    <div className="now-playing" title="Currently playing">
      <div className="np-cover"><Icon.note /></div>
...
function Button({ variant = 'default', onClick, children, icon = true }) {
  return (
    <button className={`btn ${variant}`} onClick={onClick}>
      <span>{children}</span>
      {icon && <span className="arrow"><Icon.arrow /></span>}
    </button>
  );
}

Object.assign(window, { RouterProvider, useRouter, ThemeProvider, useTheme, Nav, Footer, NowPlaying, Button });
