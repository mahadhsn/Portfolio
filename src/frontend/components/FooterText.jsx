...
const FooterText = () => {
      ...
      fetch("https://api.github.com/repos/mahadhsn/Portfolio/commits?per_page=1")
        ...
        .then((data) => {
          if (data && data.length > 0) {
            const date = new Date(data[0].commit.committer.date).toLocaleDateString("en-GB");
            setLastUpdated(date);
            localStorage.setItem("lastUpdatedCache", JSON.stringify({
              date,
              timestamp: Date.now()
            }));
          }
  ...
  return (
    <div className="text-center text-accentlight dark:text-accentdark">
        <p>mahadhssn &copy; {new Date().getFullYear()} | Updated: {lastUpdated || "Loading..."}
        </p>
    </div>
  )
}
