...
The interesting constraint was that the rescue drone had no prior knowledge of the map. It had to explore, avoid obstacles, and make decisions in real time. I modelled this using a state machine combined with the Strategy pattern, so the drone's behaviour could switch dynamically based on what it sensed.
