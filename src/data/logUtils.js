...
function parseLog(slug, raw) {
  ...
  const [datePart, readPart, tagPart] = metaMatch ? metaMatch[1].split(' · ') : [];

  const rawDate = datePart?.trim() ?? '';
  const parsed = rawDate ? new Date(rawDate) : new Date(0);
  const display = !isNaN(parsed) && rawDate
    ? parsed.toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }).toUpperCase()
    : rawDate.toUpperCase();
